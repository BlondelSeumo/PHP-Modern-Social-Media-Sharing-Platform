...
function cl_get_profile_reports($args = array()) {
	global $db;
    ...
    if (cl_queryset($reports)) {
        foreach ($reports as $row) {
            $row['u1_url']    = cl_link(cl_strf('@%s', $row['u1_username']));
            $row['u2_url']    = cl_link(cl_strf('@%s', $row['u2_username']));
...
function cl_get_total_profile_reports() {
	global $db;
...
function cl_admin_get_account_report_data($report_id = false) {
    ...
    if (cl_queryset($report)) {
        $report['url']     = cl_link(cl_strf('@%s', $report['username']));
