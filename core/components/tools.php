...
function cl_svg($icon_name = "") {
	global $config;

	$path1 = cl_strf("themes/%s/statics/ionicons/%s.svg",$config['theme'],$icon_name);
    $path2 = cl_strf("themes/%s/statics/ionicons/v5/%s.svg",$config['theme'],$icon_name);
...
function cl_import_image($data = array()) {
    ...
    try {
        ...
        return ((file_exists(cl_full_path($file_name))) ? $file_name : false); 
    } 
        ...
        try {
            $text = preg_replace_callback('/(?P<url>https?:\/\/(?:www\.|(?!www))[a-zA-Z0-9][a-zA-Z0-9-]+[a-zA-Z0-9]\.[^\s]{2,}|www\.[a-zA-Z0-9][a-zA-Z0-9-]+[a-zA-Z0-9]\.[^\s]{2,}|https?:\/\/(?:www\.|(?!www))[a-zA-Z0-9]+\.[^\s]{2,}|www\.[a-zA-Z0-9]+\.[^\s]{2,})/is', function($m) {
                if (isset($m['url'])) {
                    ...
                    return cl_html_el('a', $m['url'],array(
                        'href' => $url,
                        'target' => '_blank',
                        'class' => 'inline-link'
                    ));
...
function cl_json_server500_err($errno = false, $errstr = "", $errfile = "", $errline = "") {
    if ($errno) {
        $errors = array(
            'status'   => 500,
            'errno'    => $errno,
            'messsage' => "$errstr in [$errfile] at line $errline"
        );
    }
    else {
        $errors = array(
