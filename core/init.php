...
if (cl_is_logged() == true) {
    
    $session_id         = (not_empty($_SESSION['user_id'])) ? $_SESSION['user_id'] : $_COOKIE['user_id'];
    $cl['user_session'] = cl_get_userfromsession_id($session_id);

    if (empty($cl['user_session']) && not_empty($_POST['access_token'])) {
        header('Content-Type: application/json');
        echo json(array('status' => 400,"error" => 'Invalid access token'),1);
        exit();
    }

    $cl['hash_session'] = $session_id;
    $user_data_         = cl_user_data($cl['user_session']);
